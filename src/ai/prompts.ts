...
export function buildSelectionPrompt(items: NewsItem[], recentTitles: string[] = []): string {
  ...
  const recentBlock = recentTitles.length
    ? `Останні теми чернеток/постів (НЕ ПОВТОРЮЙ схожий сюжет чи той самий кут):
${recentTitles.map((title, i) => `${i + 1}. ${title}`).join("\n")}

Якщо останні пости про транспорт / «оновлені правила» / статус S — обери ІНШУ тему:
бізнес, житло, робота, страховка/податки, школа, курьоз, конкретна локальна новина з цифрами.`
    : `Різноманіття: не зациклюйся на транспорті й «правилах». Чергуй теми.`;
...
Пріоритети (від найвищого, АЛЕ з ротацією — не бери ту саму тему підряд):
...
- повторення теми з блоку «останні теми» вище
...
- takeaway: обов'язковий інсайт; оформлення <blockquote> додасть шаблон — ти пишеш лише текст без 💡 і лапок.
...
- takeaway: інсайт 40–110 символів БЕЗ емодзі 💡 і БЕЗ лапок на початку/в кінці (шаблон обгорне в blockquote)
...
Кандидати (змішані за темами для різноманіття):
...
3) ОБОВ'ЯЗКОВО <blockquote>💡 …</blockquote> — інсайт у цитаті (без зайвих лапок у тексті)
